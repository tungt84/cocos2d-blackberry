...
NS_CC_BEGIN
    ...
    void SmaatoLoader::downloadImage(HttpClient* client, HttpResponse* response, char* target,
            vector<char*>* beacons)
    {
    ...
    void SmaatoLoader::dowloadImage(const char* url, char* target, std::vector<char*>* beacons)
    {
        cocos2d::network::HttpRequest* request = new cocos2d::network::HttpRequest();
        request->setUrl(url);
        request->setRequestType(HttpRequest_GET);
